...
├── auth-role-redirects.spec.ts # Role-based login/signup redirects
...
- Successful login redirects to owner dashboard (needs owner test user)

### `auth-role-redirects.spec.ts`

Role-based authentication tests:

- Tenant login redirects to tenant dashboard (needs tenant test user)
- Owner signup redirects to owner dashboard after verification
- Tenant signup redirects to tenant dashboard after verification
...
- Complete login flow (needs owner test user)
- 2FA flow (skipped - needs test user with 2FA)
- Session persistence (needs owner test user)
...
- Create test users matching `TEST_CREDENTIALS.owner` and `TEST_CREDENTIALS.tenant`

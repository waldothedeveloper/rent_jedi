...
export default function TenantBasicInfoError({ error, reset }: ErrorProps) {
  useEffect(() => {
    console.error("Tenant basic info error:", error);
  }, [error]);
      ...
      <div className="flex w-full max-w-2xl flex-col gap-6 text-center">
        <div className="flex flex-col gap-2">
          ...
          <p className="text-sm text-muted-foreground">
            {error.message ||
              "We encountered an error while loading the tenant form. Please try again."}
          </p>
        </div>
